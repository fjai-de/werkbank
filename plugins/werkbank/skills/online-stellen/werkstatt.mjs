...
const ende = (t, code = 1) => { console.error(t); process.exit(code); };
...
if (args[0] === "einrichten") {
  ...
  let z; try { z = JSON.parse(fs.readFileSync(DATEI, "utf8")); } catch { ende("Noch nicht eingerichtet: node werkstatt.mjs einrichten <adresse-vom-leiter>", 3); }
  const ruf = async (m, p, b) => { const r = await fetch(z.url + p, { method: m, headers: { authorization: "Bearer " + z.code, "content-type": "application/json" }, body: b ? JSON.stringify(b) : undefined }).catch(() => null);
    if (!r) ende("Leitstand nicht erreichbar: " + z.url); const j = await r.json().catch(() => ({})); if (!r.ok) ende("Abgelehnt: " + (j.fehler || r.status)); return j; };

  const herkunft = lauf("git", ["remote", "get-url", "origin"]); const m = /github\.com[:/]([^/]+)\/([^/]+?)(\.git)?$/.exec(herkunft || "");
  if (!m) ende("Dieser Ordner hat kein GitHub-Repository (git remote origin fehlt). Erst: gh repo create <name> --private --source . --push");
  const repo = m[1] + "/" + m[2];
  ...
  if (args[0] === "status") { console.log(JSON.stringify(await ruf("GET", "/api/status?repo=" + encodeURIComponent(repo)), null, 2)); }
    ...
    if (lauf("git", ["status", "--porcelain"])) ende("Es gibt Aenderungen, die nicht committet sind. Erst committen und pushen — veroeffentlicht wird, was auf GitHub liegt.");
    const zweig = lauf("git", ["branch", "--show-current"]) || "main";
    if (lauf("git", ["rev-parse", "HEAD"]) !== lauf("git", ["rev-parse", "origin/" + zweig])) ende("Der letzte Commit ist noch nicht gepusht: git push");
    ...
    if (!da.includes(kern.split(" ")[1])) { if (lauf("gh", ["api", "-X", "POST", `repos/${repo}/keys`, "-f", "title=werkstatt-server (nur lesen)", "-f", "key=" + ich.schluessel, "-F", "read_only=true"]) === null) ende("Konnte den Leseschluessel nicht eintragen. Bist du mit gh angemeldet und gehoert dir das Repo?"); console.error("Leseschluessel eingetragen."); }
  ...
  } else ende("Aufruf: werkstatt.mjs einrichten <adresse> | veroeffentlichen [--name x] [--port n] [--art …] | status");
}
